...
function getRecuresion(callback) {
    return function recursion(dir) {
        if (!fsPlus.existsSync(dir) || !fsPlus.isDirectorySync(dir)) {
            return
        }
...
class ServerAliOS {
    constructor(sb) {
        ...
        getRecuresion(filePath => {
            ...
            try {
                require('./' +
                    modulePath.replace(path.win32.sep, path.posix.sep))(
                    router,
                    app
                )
            } catch (err) {
        ...
        getRecuresion(filePath => {
            let modulePath = path
                ...
                prefix = `/${path.posix.relative(
                    'socket',
                    modulePath
                )}`.replace(/\.js$/gi, '')
            io.of(prefix).on('connection', socket =>
                require('./' + modulePath)(socket, io)
            )
